...
const ROCCurveChart = ({ results, isDarkMode = false }) => {
  const [geminiAnalysis, setGeminiAnalysis] = useState({});
  const [isAnalyzing, setIsAnalyzing] = useState({});
  const [expandedAnalysis, setExpandedAnalysis] = useState({});

  if (!results || results.length === 0) {
    return <div className="chart-placeholder">No ROC curve data available</div>;
  }

  // ROC curve verisi olan sonuçları filtrele
  const resultsWithROC = results.filter(r => {
    const locations = [
      r.plot_data?.roc_curve,
      r.enhanced_results?.visualization_data?.roc_curve,
      r.enhanced_results?.plot_data?.roc_curve,
      r.plotData?.roc_curve
    ];
    ...
    return locations.some(location =>
      ...
      location.fpr &&
      location.tpr &&
      Array.isArray(location.fpr) &&
      Array.isArray(location.tpr)
    );
  });

  if (resultsWithROC.length === 0) {
    return (
      <div className="roc-curve-container">
        <h4>ROC Curve Analysis</h4>
        <div className="no-roc-data">
          <p>ROC curve data not available</p>
          <p>ROC curves are only generated for binary classification problems with probability predictions</p>
        </div>
      </div>
    );
  }

  const renderSingleROCCurve = (result) => {
    let rocData = null;
    let prData = null;

    const locations = [
      result.plot_data,
      result.enhanced_results?.visualization_data,
      result.enhanced_results?.plot_data,
      result.plotData
    ];

    for (const location of locations) {
      if (location?.roc_curve && location.roc_curve.fpr && location.roc_curve.tpr) {
        rocData = location.roc_curve;
        break;
      }
    }

    for (const location of locations) {
      if (location?.precision_recall_curve && location.precision_recall_curve.precision && location.precision_recall_curve.recall) {
        prData = location.precision_recall_curve;
        break;
      }
    }
    ...
    const rocAUC = getMetricValue(result, 'roc_auc') ||
                   result.metrics?.['ROC AUC'] ||
                   result.metrics?.roc_auc || 0;

    // ROC Curve Chart Data
    const rocChartData = {
      labels: rocData.fpr.map((_, i) => i), // Index labels
      datasets: [
        {
          label: `${result.modelName} (AUC = ${rocAUC.toFixed(3)})`,
    ...
    // Precision-Recall Chart Data (if available)
    let prChartData = null;
    if (prData) {
      prChartData = {
        labels: prData.recall.map((_, i) => i),
        datasets: [
          {
            label: `${result.modelName} (Precision-Recall)`,
            data: prData.recall.map((recall, i) => ({ x: recall, y: prData.precision[i] })),
    ...
    return (
      <div key={resultId} className="single-roc-curve">
        {/* ROC Info */}
        <div className="roc-info">
          ...
          <div className="roc-stats">
            <span className="stat-chip">
              ...
              <span className="stat-value">{rocAUC.toFixed(4)}</span>
            </span>
            <span className="stat-chip">
              <span className="stat-label">Classification:</span>
              <span className="stat-value">Binary</span>
            </span>
          </div>
        </div>

        {/* ROC Curve Chart */}
        <div className="roc-charts-grid">
        ...
        <div className="curve-analysis">
          <div className="analysis-stats">
            <div className="analysis-stat">
              ...
              <span className={`value ${rocAUC >= 0.9 ? 'excellent' : rocAUC >= 0.8 ? 'good' : rocAUC >= 0.7 ? 'fair' : 'poor'}`}>
                {rocAUC >= 0.9 ? 'Excellent' : rocAUC >= 0.8 ? 'Good' : rocAUC >= 0.7 ? 'Fair' : 'Poor'}
              </span>
            </div>
            <div className="analysis-stat">
              <span className="label">Better than Random:</span>
              <span className="value">{rocAUC > 0.5 ? 'Yes' : 'No'}</span>
            </div>
            <div className="analysis-stat">
              <span className="label">Data Points:</span>
              <span className="value">{rocData.fpr.length}</span>
            </div>
          </div>
        </div>

        {/* ROC Curve Gemini Analysis */}
        <div className="gemini-analysis-section">
          <button
            className="gemini-explain-btn"
            onClick={async () => {
              setIsAnalyzing(prev => ({ ...prev, [resultId]: true }));
              const rocAnalysisData = {
                model: result.modelName,
                dataset: result.datasetName,
                roc_auc: rocAUC,
                roc_curve: {
                ...
                precision_recall: prData ? {
                  precision: prData.precision,
                  recall: prData.recall
                } : null,
                performance_level: rocAUC >= 0.9 ? 'Excellent' : rocAUC >= 0.8 ? 'Good' : rocAUC >= 0.7 ? 'Fair' : 'Poor'
              };
              const analysis = await callGeminiAnalysis(rocAnalysisData, 'roc_curve', `${result.modelName} modeli için ROC curve ve classification performance analizi`);
              setGeminiAnalysis(prev => ({ ...prev, [resultId]: analysis }));
              setIsAnalyzing(prev => ({ ...prev, [resultId]: false }));
              setExpandedAnalysis(prev => ({ ...prev, [resultId]: true }));
            }}
            disabled={isAnalyzing[resultId]}
          >
            {isAnalyzing[resultId] ? '🤖 Analiz ediliyor...' : '🤖 Explain With Gemini'}
  ...
  return (
    <div className="roc-curve-container">
      <h4>ROC Curve Analysis</h4>
      <div className="roc-curves-grid">
        {resultsWithROC.map(result => renderSingleROCCurve(result))}
      </div>
    </div>
  );
};
...
export { PerformanceChart, ConfusionMatrix, ComparisonTable, ROCCurveChart };
