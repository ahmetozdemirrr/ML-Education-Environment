...
def load_and_prepare_data(
    ...
    apply_scaling_setting = global_settings.get("applyFeatureScaling", True) # Frontend'den bu ayar gelmeli
    scaler_type_setting = global_settings.get("scalerType", 'standard') # Frontend'den bu ayar gelmeli
    ...
    if global_settings.get('useTrainTestSplit', False):
        ...
        random_state_val = global_settings.get('randomSeed')  # None olabilir
        ...
        # Eğer burada yine de ölçekleme isteniyorsa (Pipeline kullanılmayacaksa), tüm X ölçeklenir.
        if scaler: # Eğer ölçeklendirme ayarı açıksa ve CV seçiliyse, tüm veriyi ölçekle (Pipeline daha iyi!)
            ...
            processed_data["data_preparation_log"].append(
                f"Cross-validation için tüm veri. Özellikler {scaler_type_setting}Scaler ile ölçeklendi (UYARI: Bu, CV'de veri sızıntısına neden olabilir. Pipeline kullanılması önerilir)."
            )
        else: # Ölçeklendirme kapalıysa, orijinal X_full kalır
            processed_data["data_preparation_log"].append(
